...
Приложение работает только с одним настроенным листом. Отдельный лист со справочником типов не
создаётся и не требуется: категоризация встроена в приложение.
...
обратная логистика, возвраты и отмены, реклама, эквайринг, хранение, упаковка, другие услуги и
компенсации. Только неизвестные приложению типы попадают в «Неопознанные начисления». В таком
случае предупреждение в логе содержит исходные `type_id` и `type_name`. Туда же записывается сумма
операции, если Ozon не передал её детализацию.
