...
Контейнер загрузит начисления в выбранный лист и завершится. Заголовки займут строку `A1:W1`,
...
обратная логистика, возвраты и отмены, реклама, эквайринг, хранение, упаковка, приёмка поставки,
утилизация, кросс-докинг и компенсации. Только неизвестные приложению типы попадают в
«Неопознанные начисления». В таком
...
расхождении данные за текущую дату не записываются, а запуск завершается с кодом `1`. Проверьте
предупреждения и ошибки в `logs/logs.log`, затем сравните итог за период с финансовым отчётом в
кабинете Ozon.
...
выгрузки внутри диапазона `A:W` могут быть перезаписаны.
...
Период обрабатывается последовательно, по одному дню. Если при обработке одного из дней возникает
ошибка, данные за предыдущие дни остаются в таблице, а данные за день с ошибкой не записываются.
Чтобы продолжить выгрузку после устранения ошибки, укажите дату из лога в `OZON_DATE_FROM`.
...
Контейнеру нужны права на чтение `secrets/google-service-account.json` и на запись в `logs/`.
...
| `Set exactly one of ...` | Задана только одна из переменных: `GOOGLE_CREDENTIALS_PATH` или `GOOGLE_CREDENTIALS_JSON` |
| Google 403 | Включён Google Sheets API, а адрес из поля `client_email` добавлен в таблицу с ролью «Редактор» |
...
| `Permission denied` в Docker | Процесс с UID/GID `10001` может читать `secrets/` и записывать данные в `logs/` |
...
Можем помочь на коммерческой основе с развёртыванием проекта на вашем сервере или сделать всё
«под ключ»: подготовить инфраструктуру, настроить и администрировать проект. Обращайтесь:
...
- можно за деньги развернуть проект на сервере клиента для использования самим клиентом;
...
использования приложения, в том числе из-за ошибок или расхождений в выгруженных данных. Сверяйте
результат с финансовыми отчётами Ozon.
